...
**How**: Copies the hook file from `templates-shared/hooks/` to the project's `api-services/hooks/` directory. Core hooks (`useAuth`, `useNotification`) cannot be removed.
...
**How**: Supports resetting specific files/folders or everything. Uses fuzzy matching — `reex reset useAuth` finds `hooks/useAuth.ts`. Handles ambiguous targets with helpful error messages.
...
### 36. Hook Repository
**What**: A library of installable utility hooks (`useAuth`, `useNotification`, and more).
