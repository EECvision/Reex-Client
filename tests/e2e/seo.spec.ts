...
test.describe("SEO without JavaScript", () => {
  ...
  test("homepage serves canonical metadata and structured data", async ({ page }) => {
    ...
    expect(schema["@graph"][0].alternateName).toContain("Reex Studio");
    expect(schema["@graph"][0].about["@id"]).toBe("https://www.reex-api.dev/#software");
    expect(schema["@graph"][1].isPartOf["@id"]).toBe("https://www.reex-api.dev/#software");
  });

  test("homepage content is available before JavaScript runs", async ({ page }) => {
    await page.goto("/?localPort=4000&utm_source=seo-check");
  ...
  test("support has its own metadata and all FAQ answers work without JavaScript", async ({ page }) => {
    ...
    await expect(page).toHaveTitle("Help & API Troubleshooting | Reex API Studio");
  ...
  test("personal workspace pages are noindex and have distinct titles", async ({ page }) => {
    for (const [path, title] of [["/settings", "Workspace Settings"], ["/sandbox", "API Sandbox"]]) {
      ...
      await expect(page).toHaveTitle(`${title} | Reex API Studio`);
...
test("mobile visitors can read the homepage and dismiss the desktop tip", async ({ page }) => {
  ...
  // Initial bridge discovery can take longer than Playwright's default 5s.
