...
interface AuthModalProps {
    ...
    onSave: (collectionId: string, token: string, customHeaders: Record<string, string>) => void;
}
...
export const AuthModal: React.FC<AuthModalProps> = ({
    ...
    const [draftHeaders, setDraftHeaders] = useState<{ id: string, key: string, value: string }[]>([]);
    ...
    useEffect(() => {
        ...
        if (col) {
            ...
            const headersArray = Object.entries(headers).map(([key, value], index) => ({
    ...
    const hasAuth = (col: StandaloneCollection) => {
        const auth = col.config?.auth;
        return !!(auth?.token || (auth?.customHeaders && Object.keys(auth.customHeaders).length > 0));
    };
                    ...
                    <div className={styles.collectionList}>
                        {collections.map(col => (
                            ...
                            >
                                ...
                                {hasAuth(col) && <div className={styles.authBadge} title="Auth configured" />}
                            </div>
                        ))}
                        {collections.length === 0 && (
                            <div style={{ padding: '0 16px', fontSize: '13px', color: '#9ca3af', fontStyle: 'italic' }}>
