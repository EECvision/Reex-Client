...
import { PRO_RECENT_COLLECTION_LIMIT } from '@/lib/constants';
...
export async function addToHistory(name: string, content: any) {
    ...
    try {
        ...
        if (items && items.length > PRO_RECENT_COLLECTION_LIMIT) {
            const itemsToDelete = items.slice(PRO_RECENT_COLLECTION_LIMIT).map(i => i.id);
...
export async function getHistory() {
    ...
    const { data, error } = await supabase
        ...
        .limit(PRO_RECENT_COLLECTION_LIMIT);
